...
public final class Linux_SambaHostHelper {
	...
	*/	
	public final static Linux_SambaHost getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_SambaHostHelper.getInstance(cimClient, new Linux_SambaHost(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaHost getInstance(CIMClient cimClient, Linux_SambaHost dataInstance) {
		...
		return Linux_SambaHostHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaHost getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_SambaHost(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_SambaHost createInstance(CIMClient cimClient, Linux_SambaHost dataInstance, boolean notifyDifferencies){ 
		...
		Linux_SambaHost newDataInstance = null;
		try {
			newDataInstance = Linux_SambaHostHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_SambaHost modifyInstance(CIMClient cimClient, Linux_SambaHost dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_SambaHostHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_SambaHost dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_SambaHost dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_SambaHost.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_SambaHost.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_SambaHost.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_SambaHost.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_SambaHost.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
