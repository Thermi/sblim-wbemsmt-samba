...
public final class Linux_SambaLoggingForGlobalHelper {

	public final static String CIM_CLASS_NAME = "Linux_SambaLoggingForGlobal";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_SambaLoggingForGlobal(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<Linux_SambaLoggingForGlobal.Java_Package_List.size(); i++) {
						if (!((String)(Linux_SambaLoggingForGlobal.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_SambaLoggingForGlobal.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_SambaLoggingForGlobal.Java_Package_List.setElementAt((String)(Linux_SambaLoggingForGlobal.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_SambaLoggingForGlobal.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_SambaLoggingForGlobal getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_SambaLoggingForGlobalHelper.getInstance(cimClient, new Linux_SambaLoggingForGlobal(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaLoggingForGlobal getInstance(CIMClient cimClient, Linux_SambaLoggingForGlobal dataInstance) {
		...
		return Linux_SambaLoggingForGlobalHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaLoggingForGlobal getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_SambaLoggingForGlobal(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_SambaLoggingForGlobal createInstance(CIMClient cimClient, Linux_SambaLoggingForGlobal dataInstance, boolean notifyDifferencies){ 
		...
		Linux_SambaLoggingForGlobal newDataInstance = null;
		try {
			newDataInstance = Linux_SambaLoggingForGlobalHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_SambaLoggingForGlobal modifyInstance(CIMClient cimClient, Linux_SambaLoggingForGlobal dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_SambaLoggingForGlobalHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_SambaLoggingForGlobal dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_SambaLoggingForGlobal dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_SambaLoggingForGlobal.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_SambaLoggingForGlobal.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_SambaLoggingForGlobal.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_SambaLoggingForGlobal.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_SambaLoggingForGlobal.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_SambaLoggingOptions(Linux_SambaLoggingOptions Linux_SambaLoggingOptions) {
		return true;
	}
