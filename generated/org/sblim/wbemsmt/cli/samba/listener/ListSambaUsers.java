/**
 * ListSambaUsers.java Â© Copyright IBM Corp.  2009,2006,2007 THIS FILE IS PROVIDED UNDER THE TER MS OF
...
public class ListSambaUsers extends CimCommand {
    ...
    public static final OptionDefinition KEY_list = new OptionDefinition(null, "list",
            "INSTANCE_WITH_CHILDS", "ListSambaUsers.list.argValue", false, false,
            "ListSambaUsers.list.argDescription");
            ...
            values.getOut().println(
                    "\n"
                            + bundle.getString("listing", new Object[] { bundle
                                    .getString("UserListDataContainer.caption") }));

            CliDataLoader loader = new ListSambaUsersLoader();
            loader.load(bundle, adapter, commandValues);

            org.sblim.wbemsmt.cli.samba.container.user.UserListDataContainerImpl dc = new org.sblim.wbemsmt.cli.samba.container.user.UserListDataContainerImpl(
                    adapter);

            int count = adapter.count("users",
                    org.sblim.wbemsmt.samba.bl.container.user.UserListItemDataContainer.class, dc);
            dc.getUsers().clear();
            for (int i = 0; i < count; i++) {
                org.sblim.wbemsmt.cli.samba.container.user.UserListItemDataContainerImpl child = new org.sblim.wbemsmt.cli.samba.container.user.UserListItemDataContainerImpl(
                        adapter);
    ...
    private void setKeyValues(CommandLine cmd, AbstractBaseCimAdapter adapter,
            org.sblim.wbemsmt.samba.bl.container.user.UserListDataContainer dc)
