...
public class UserInPrinterACLDataContainerImpl extends org.sblim.wbemsmt.tools.jsf.EditBasePanel
        implements org.sblim.wbemsmt.samba.bl.container.printer.UserInPrinterACLDataContainer,
    ...
    public UserInPrinterACLDataContainerImpl(
    ...
    private void addUsers(
            org.sblim.wbemsmt.jsf.samba.container.printer.UserACLItemDataContainerForPrinter_AsUsers_InUserInPrinterACLDataContainerImpl child) {
    ...
    public java.util.List getUsersForUI() {
        ...
        while (result.size() < MIN_TABLE_LENGTH) {
            try {
                org.sblim.wbemsmt.jsf.samba.container.printer.UserACLItemDataContainerForPrinter_AsUsers_InUserInPrinterACLDataContainerImpl item = new org.sblim.wbemsmt.jsf.samba.container.printer.UserACLItemDataContainerForPrinter_AsUsers_InUserInPrinterACLDataContainerImpl(
    ...
    public void reload() {
        ...
        if (ic_usr_CurrentForceUser != null) {
            ic_usr_CurrentForceUser.setLabelText(bundle
                    .getString("UserInPrinterACLDataContainer.currentForceUser"));
        }
        if (ic_usr_NewForceUser != null) {
            ic_usr_NewForceUser.setLabelText(bundle
                    .getString("UserInPrinterACLDataContainer.newForceUser"));
        }
    }
    ...
    public void countAndCreateChildren() throws WbemsmtException {
        ...
        try {
            int count = adapter
                    .count(
                            "users",
                            org.sblim.wbemsmt.samba.bl.container.printer.UserACLItemDataContainerForPrinter.class,
                            this);
            if (count != usersCount) {
                usersCount = count;
                clearUsers();
                for (int i = 0; i < count; i++) {
                    addUsers(new org.sblim.wbemsmt.jsf.samba.container.printer.UserACLItemDataContainerForPrinter_AsUsers_InUserInPrinterACLDataContainerImpl(
                            (org.sblim.wbemsmt.samba.bl.adapter.SambaCimAdapter) adapter,
                            bindingPrefix, i));
                }
            }
