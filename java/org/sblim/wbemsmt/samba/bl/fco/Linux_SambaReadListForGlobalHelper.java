...
public final class Linux_SambaReadListForGlobalHelper {

	public final static String CIM_CLASS_NAME = "Linux_SambaReadListForGlobal";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_SambaReadListForGlobal(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<Linux_SambaReadListForGlobal.Java_Package_List.size(); i++) {
						if (!((String)(Linux_SambaReadListForGlobal.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_SambaReadListForGlobal.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_SambaReadListForGlobal.Java_Package_List.setElementAt((String)(Linux_SambaReadListForGlobal.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_SambaReadListForGlobal.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_SambaReadListForGlobal getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_SambaReadListForGlobalHelper.getInstance(cimClient, new Linux_SambaReadListForGlobal(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaReadListForGlobal getInstance(CIMClient cimClient, Linux_SambaReadListForGlobal dataInstance) {
		...
		return Linux_SambaReadListForGlobalHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaReadListForGlobal getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_SambaReadListForGlobal(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_SambaReadListForGlobal createInstance(CIMClient cimClient, Linux_SambaReadListForGlobal dataInstance, boolean notifyDifferencies){ 
		...
		Linux_SambaReadListForGlobal newDataInstance = null;
		try {
			newDataInstance = Linux_SambaReadListForGlobalHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_SambaReadListForGlobal modifyInstance(CIMClient cimClient, Linux_SambaReadListForGlobal dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_SambaReadListForGlobalHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_SambaReadListForGlobal dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_SambaReadListForGlobal dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_SambaReadListForGlobal.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_SambaReadListForGlobal.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_SambaReadListForGlobal.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_SambaReadListForGlobal.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_SambaReadListForGlobal.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
