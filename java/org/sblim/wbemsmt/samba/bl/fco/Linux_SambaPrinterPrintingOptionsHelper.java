...
public final class Linux_SambaPrinterPrintingOptionsHelper {
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<Linux_SambaPrinterPrintingOptions.Java_Package_List.size(); i++) {
						if (!((String)(Linux_SambaPrinterPrintingOptions.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_SambaPrinterPrintingOptions.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_SambaPrinterPrintingOptions.Java_Package_List.setElementAt((String)(Linux_SambaPrinterPrintingOptions.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_SambaPrinterPrintingOptions.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_SambaPrinterPrintingOptions getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_SambaPrinterPrintingOptionsHelper.getInstance(cimClient, new Linux_SambaPrinterPrintingOptions(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaPrinterPrintingOptions getInstance(CIMClient cimClient, Linux_SambaPrinterPrintingOptions dataInstance) {
		...
		return Linux_SambaPrinterPrintingOptionsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaPrinterPrintingOptions getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_SambaPrinterPrintingOptions(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_SambaPrinterPrintingOptions createInstance(CIMClient cimClient, Linux_SambaPrinterPrintingOptions dataInstance, boolean notifyDifferencies){ 
		...
		Linux_SambaPrinterPrintingOptions newDataInstance = null;
		try {
			newDataInstance = Linux_SambaPrinterPrintingOptionsHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_SambaPrinterPrintingOptions modifyInstance(CIMClient cimClient, Linux_SambaPrinterPrintingOptions dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_SambaPrinterPrintingOptionsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_SambaPrinterPrintingOptions dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_SambaPrinterPrintingOptions dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_SambaPrinterPrintingOptions.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_SambaPrinterPrintingOptions.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_SambaPrinterPrintingOptions.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_SambaPrinterPrintingOptions.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_SambaPrinterPrintingOptions.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
