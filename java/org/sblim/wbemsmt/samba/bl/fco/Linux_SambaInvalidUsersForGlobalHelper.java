		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					for (int i=0; clazz==null && i<Linux_SambaInvalidUsersForGlobal.Java_Package_List.size(); i++) {
						if (!((String)(Linux_SambaInvalidUsersForGlobal.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_SambaInvalidUsersForGlobal.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_SambaInvalidUsersForGlobal.Java_Package_List.setElementAt((String)(Linux_SambaInvalidUsersForGlobal.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_SambaInvalidUsersForGlobal.Java_Package_List.get(i)) + cimClassName;
