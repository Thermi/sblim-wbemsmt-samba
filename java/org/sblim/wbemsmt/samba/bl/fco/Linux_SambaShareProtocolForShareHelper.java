...
public final class Linux_SambaShareProtocolForShareHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_SambaShareProtocolForShare(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<Linux_SambaShareProtocolForShare.Java_Package_List.size(); i++) {
						if (!((String)(Linux_SambaShareProtocolForShare.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_SambaShareProtocolForShare.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_SambaShareProtocolForShare.Java_Package_List.setElementAt((String)(Linux_SambaShareProtocolForShare.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_SambaShareProtocolForShare.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_SambaShareProtocolForShare getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_SambaShareProtocolForShareHelper.getInstance(cimClient, new Linux_SambaShareProtocolForShare(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaShareProtocolForShare getInstance(CIMClient cimClient, Linux_SambaShareProtocolForShare dataInstance) {
		...
		return Linux_SambaShareProtocolForShareHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaShareProtocolForShare getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_SambaShareProtocolForShare(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_SambaShareProtocolForShare createInstance(CIMClient cimClient, Linux_SambaShareProtocolForShare dataInstance, boolean notifyDifferencies){ 
		...
		Linux_SambaShareProtocolForShare newDataInstance = null;
		try {
			newDataInstance = Linux_SambaShareProtocolForShareHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_SambaShareProtocolForShare modifyInstance(CIMClient cimClient, Linux_SambaShareProtocolForShare dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_SambaShareProtocolForShareHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_SambaShareProtocolForShare dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_SambaShareProtocolForShare dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_SambaShareProtocolForShare.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_SambaShareProtocolForShare.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_SambaShareProtocolForShare.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_SambaShareProtocolForShare.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_SambaShareProtocolForShare.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_SambaShareProtocolOptions(Linux_SambaShareProtocolOptions Linux_SambaShareProtocolOptions) {
		return true;
	}
