...
public final class Linux_SambaBrowseOptionsHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_SambaBrowseOptions(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = null;
					String cimClassName = cimInstance.getClassName();
				
					for (int i=0; clazz==null && i<Linux_SambaBrowseOptions.Java_Package_List.size(); i++) {
						if (!((String)(Linux_SambaBrowseOptions.Java_Package_List.get(i))).trim().equals("") && //$NON-NLS-1$
								!((String)(Linux_SambaBrowseOptions.Java_Package_List.get(i))).endsWith(".")) { //$NON-NLS-1$
							Linux_SambaBrowseOptions.Java_Package_List.setElementAt((String)(Linux_SambaBrowseOptions.Java_Package_List.get(i)) + ("."), i); //$NON-NLS-1$
						}
						cimClassName = (Linux_SambaBrowseOptions.Java_Package_List.get(i)) + cimClassName;
	...
	*/	
	public final static Linux_SambaBrowseOptions getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_SambaBrowseOptionsHelper.getInstance(cimClient, new Linux_SambaBrowseOptions(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaBrowseOptions getInstance(CIMClient cimClient, Linux_SambaBrowseOptions dataInstance) {
		...
		return Linux_SambaBrowseOptionsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_SambaBrowseOptions getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			dataInstance = new Linux_SambaBrowseOptions(cimObjectPath, cimInstance);
		} catch (Exception e) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_SambaBrowseOptions.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_SambaBrowseOptions.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_SambaBrowseOptions.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_SambaBrowseOptions.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Browsable(Boolean Browsable) {
		return true;
	}
